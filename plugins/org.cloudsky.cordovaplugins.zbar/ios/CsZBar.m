...
- (void)scan: (CDVInvokedUrlCommand*)command;
{
    if(self.scanInProgress) {
        ...
        [self.scanReader.scanner setSymbology: ZBAR_CODE128
                        config: ZBAR_CFG_ENABLE
                        to: 1];
        //[self.scanReader.scanner setSymbology:ZBAR_CODE128 config:ZBAR_CFG_X_DENSITY to:0];
        //[self.scanReader.scanner setSymbology:ZBAR_CODE128 config:ZBAR_CFG_Y_DENSITY to:2];

        self.scanReader.readerView.zoom = 1.0;
        self.scanReader.readerView.scanCrop = (CGRect){{ 0, 0.3 }, { 1, 0.4 }};
        //self.scanReader.readerView.session.canSetSessionPreset;
